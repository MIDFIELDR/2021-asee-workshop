...
Download the `prepared_data_graph_basics.zip` file from the workshop website with the following code. 
...
- Move the files into the top level of the `data` folder, which should now include:    
...
The midfieldr `student` table has been summarized by counting the number of students by race/ethnicity and sex, omitting "International" and "Other/Unknown" race values. These data, `student_demogr.rds` are part of the prepared data from the `.zip` file you downloaded earlier. Again,  we read the data in using `readRDS()`. 

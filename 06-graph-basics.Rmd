...
A data set has been extracted from the midfieldr `student` table with a sample of 3000 student SAT scores. Download the file using the data.table  `fread()` function as follows. (If the download fails, instructions for manually downloading the file are [here]().)

```{r}
# Download data from the workshop data folder
data_url <- paste0("https://raw.githubusercontent.com/MIDFIELDR/", 
                   "2021-asee-workshop/main/data/sat.csv")
sat <- fread(data_url)
```
...
# fwrite(DT, file = "data/student_demogr.csv")
```

Download the file from the URL given:

```{r}
data_location <- paste0("https://raw.githubusercontent.com/MIDFIELDR/", 
                   "2021-asee-workshop/main/data/student_demogr.csv")
student_demogr <- fread(data_location, colClasses = c(race_sex = "factor"))
```

If the download fails, 

- Navigate to the [Workshop source files](https://github.com/MIDFIELDR/2021-asee-workshop/blob/main/data/student_demogr.csv) on GitHub 
- Right-click on *Raw* 
- Select *Save link as...* to download and save to your `workshop/data/` directory. 

```{r}
data_location <- "data/student_demogr.txt"
student_demogr <- fread(data_location, colClasses = c(race_sex = "factor"))
```
...
If a download using `fread()` fails, 

- Navigate to the [Workshop source files](https://github.com/MIDFIELDR/2021-asee-workshop/tree/main/data) on GitHub 
- Click on the file you want, taking you to its page. 
- Right-click on *Raw* 
- Download by selecting *Save link as...* save to your `workshop/data/` directory. 

Use `fread()` as before to read the file. For example, 
...
```{r}
dir('data')
# [1] "case2_course.rds"     "case2_degree.rds"    
# [3] "case2_student.rds"    "case2_term.rds"      
# [5] "example2.xlsx"        "info_pkg.rds"        
# [7] "sat.csv"              "student_demogr.csv"  
# [9] "student_demogr.txt"   "student_demogr_2.txt"

library(zip)
zip(
  zipfile = "data/workshop_data.zip",
  files = c("data/student_demogr.txt"),
...
zip_url <- "https://github.com/MIDFIELDR/2021-asee-workshop/raw/main/data/test.zip"

download.file(zip_url, destfile = "data/download_zip.zip")

...
*Comments* are annotations to make the source code easier for humans to understand but are ignored by R. Comments in R are denoted by a hash tag `#`. 
...
```{r}
# a data frame included with midfieldr
toy_student
```
...
In data.table syntax, the same operation is as follows. Note we created a new object `air_quality` that is a data.table form of the original data frame. 
